...
export function ResultTable({ data }: { data: unknown }) {
  ...
  return (
    <section className="mt-4 overflow-hidden rounded-[10px] border border-ink/10 bg-white/70 shadow-line">
        ...
        <table className="min-w-full border-separate border-spacing-0 text-left text-sm">
          ...
          <tbody>
            {rows.map((row, rowIndex) => (
              <tr key={rowIndex} className="transition hover:bg-moss/5 odd:bg-white/45 even:bg-white/20">
                {columns.map((column) => (
