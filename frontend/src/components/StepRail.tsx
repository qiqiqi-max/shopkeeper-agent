...
function FlowNodeCard({ node, status }: { node: FlowNode; status: FlowStatus }) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex h-10 items-center gap-2 rounded-[8px] border px-3 text-sm font-semibold shadow-line transition",
...
export function StepRail({ steps = [] }: { steps?: StepState[] }) {
  ...
  return (
    <section className="mt-4 overflow-hidden rounded-[10px] border border-ink/10 bg-white/45 px-3 py-4 shadow-line">
